...
# =============================================================================
# MENU DE HERRAMIENTAS / OVERLAYS + CONTROLES DE REPLAY (Fase 2)
...
# --- Fila de controles de Replay ---
my $replay_bar = $mw->Frame(-background => $BAR_BG);
$replay_bar->pack(-side => 'top', -fill => 'x', -before => $canvas_price);
...
# =============================================================================
# CONTROLES DE REPLAY  (Iniciar | |< | Play | >| | >>)  + estado
# =============================================================================
$replay_bar->Label(-text => 'Replay:', -background => $BAR_BG,
    -foreground => '#363a45', -font => 'TkDefaultFont 9 bold')
    ->pack(-side => 'left', -padx => 8, -pady => 3);

my ( $btn_iniciar, $btn_play, $replay_status );
$btn_iniciar = $replay_bar->Button(%BBS,
    -text => 'Iniciar', -foreground => '#2962ff', -font => 'TkDefaultFont 9 bold',
    -command => sub {
        if   ( $engine->is_replay_active ) { $engine->replay_exit }
        else                               { $engine->replay_start }
    },
)->pack(-side => 'left', -padx => 2, -pady => 2);

$replay_bar->Button(%BBS, -text => '|<',
    -command => sub { $engine->replay_step(-1) })
    ->pack(-side => 'left', -padx => 1, -pady => 2);
$btn_play = $replay_bar->Button(%BBS, -text => 'Play', -font => 'TkDefaultFont 9 bold',
    -command => sub { $engine->replay_play })
    ->pack(-side => 'left', -padx => 1, -pady => 2);
$replay_bar->Button(%BBS, -text => '>|',
    -command => sub { $engine->replay_step(1) })
    ->pack(-side => 'left', -padx => 1, -pady => 2);
$replay_bar->Button(%BBS, -text => '>>',
    -command => sub { $engine->replay_fast_forward })
    ->pack(-side => 'left', -padx => 1, -pady => 2);

$replay_status = $replay_bar->Label(-text => 'inactivo', -background => $BAR_BG,
    -foreground => '#787b86', -font => 'TkDefaultFont 9')
    ->pack(-side => 'left', -padx => 10);

$replay_bar->Label(-text => 'En Replay ningun calculo usa velas futuras',
    -background => $BAR_BG, -foreground => '#787b86', -font => 'TkDefaultFont 8')
    ->pack(-side => 'right', -padx => 10);

# El engine notifica cambios de estado de replay para sincronizar la UI.
$engine->set_replay_callback( sub {
    my ( $active, $playing, $status ) = @_;
    $replay_status->configure( -text => $status );
    $btn_iniciar->configure(
        -text       => $active ? 'Salir' : 'Iniciar',
        -foreground => $active ? '#ef5350' : '#2962ff',
    );
    $btn_play->configure( -text => $playing ? 'Pause' : 'Play' );
} );
